...
gtkquery32=/usr/lib/i386-linux-gnu/libgtk2.0-0/gtk-query-immodules-2.0
gtkquery64=/usr/lib/x86_64-linux-gnu/libgtk2.0-0/gtk-query-immodules-2.0
gtkimmod32=/usr/lib/i386-linux-gnu/gtk-2.0/2.10.0/gtk.immodules
gtkimmod64=/usr/lib/x86_64-linux-gnu/gtk-2.0/2.10.0/gtk.immodules
...
usage() {
  cat <<-EOF
	Usage:
	  $0
	     [-h] [-s] [-d] atokx3_productdir atokx3up2_tarball
	     atokx3gtk216_tarball a20y1311lx_tarball [iiimf_status_hide]
	Options:
	  -h:
...
sed -i $workdir/atokx3/setupatok_tar.sh \
  -e '/^gtkquerylist32="\$list"/ i\list="$list '$gtkquery32'"' \
  -e '/^gtkquerylist64="\$list"/ i\list="$list '$gtkquery64'"' \
  -e '/^gtkimmodlist32="\$list"/ i\list="$list '$gtkimmod32'"' \
  -e '/^gtkimmodlist64="\$list"/ i\list="$list '$gtkimmod64'"' \
  || exit $?
...
# by Debian Wheezy Japanese Task
...
if [ -n "$hide_status" ]; then
  ...
  # modify the start-up script for im-switch
  sudo sed -i /etc/X11/xinit/xinput.d/iiimf \
